...
Google Sheets → Extract → Transform → Load → Supabase (PostgreSQL) → Power BI (gspread) (pandas) (REST API)
...
pipeline/ ├── extract.py → lê as 5 abas da planilha (Google Sheets API) ├── transform.py → limpa, valida e padroniza os dados ├── load.py → envia os dados para o Supabase via REST API (upsert) └── main.py → orquestra as 3 etapas do pipeline

.github/workflows/ └── sync.yml → agenda a execução automática (cron a cada 2h)
